...
interface SchoolProviderProps {
    children:       ReactNode
    initialProfile: AnyProfile | null  // ✅ AnyProfile — accepts parent and teacher
}
...
export function SchoolProvider({ children, initialProfile }: SchoolProviderProps) {
    ...
    // ── Derive school data from profile ────────────────────────────────────
    // Both parent and teacher profiles include school and curriculum
    // via the shared BaseProfile — no separate fetch needed
    useEffect(() => {
        if (!profile) {
            setSchool(undefined)
            setIsLoading(false)
        ...
        try {
            ...
            console.error('SchoolProvider: failed to derive school data:', error)
            setSchool(undefined)
        } finally {
            setIsLoading(false)
        }
    ...
    // ── Apply CSS variables when school colors are available ───────────────
    useEffect(() => {
        if (!school) return
        const root = document.documentElement
        root.style.setProperty('--school-primary',   school.primaryColor)
