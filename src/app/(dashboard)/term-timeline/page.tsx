...
export async function generateMetadata(): Promise<Metadata> {
    ...
    if (!user) return { title: "Roadmap | SchoolPaaS" };
    ...
    return {
        title: `Academic Roadmap | ${profile?.name || "Registry"} | SchoolPaaS`,
        description: "Institutional timeline and modular syllabus progression."
    };
}
...
export default async function Page() {
    ...
    const profile = await prisma.profile.findUnique({
        ...
        include: { 
            selectedSubjects: { take: 1 },
            taughtClasses: { take: 1 }
        }
    });

    if (!profile) redirect("/login");

    // 2. Resolve Subject Context for the Roadmap
    // We prioritize the first subject the user is currently leading or studying
    const targetSubjectId = profile.selectedSubjects[0]?.id;

    // 3. Rule 7 & 11: Fetch Tiered Registry Truth
    // If schoolId is null, roadmap fetches Global Core nodes.
    const roadmapData = await getGlobalTermSchedule(
        profile.schoolId, 
        1, // Defaulting to 1st Term
        targetSubjectId
    );
