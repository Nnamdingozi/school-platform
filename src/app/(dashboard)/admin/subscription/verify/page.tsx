 ...
 * Rule 11: Final System Truth - Updates the DB before rendering the UI.
 */
    ...
    if (!reference) {
        return (
            <VerifySubscriptionClient 
                initialStatus="error" 
                initialMessage="Invalid transaction reference provided by gateway." 
            />
        );
    }

    // Rule 11: Database Engine comparison happens inside this Action
    const result = await verifySubscriptionPayment(reference);
    ...
    return (
        <VerifySubscriptionClient 
            ...
            initialMessage={result.success 
                ? `License Activated: ${result.planName} plan is now active.` 
                : (result.error || "Registry synchronization failed.")
            }
        />
    );
}