 ...
 * Rule 15: Resolved Hub ID variance error via nullish normalization.
 * Rule 18: Semantic Flip (bg-background, bg-card, bg-surface).
 * Rule 19: Standardized Geometry [2rem].
...
export default function StudentDetailPage() {
    ...
    useEffect(() => {
        ...
        const fetchStudent = async () => {
            try {
                setLoading(true);
                // ✅ RESOLVED: Normalizing undefined to null for strict parameter matching
    ...
    async function handleActionSuccess() {
        ...
        if (actionState?.action === 'delete') {
            ...
            try {
                const updated = await getUserById(id, actorProfile?.schoolId ?? null);
                setUser(updated);
            } catch (error: unknown) {
                console.error(`[REGISTRY_REFRESH_FAULT]: ${getErrorMessage(error)}`);
            }
        }
    }
        ...
        <div className="min-h-screen bg-background flex items-center justify-center p-6">
            <div className="text-center space-y-6 animate-in fade-in zoom-in-95">
                ...
                <div className="space-y-2">
                    ...
                    <p className="text-muted-foreground text-[10px] font-bold uppercase tracking-widest">Student profile not discovered in registry.</p>
                </div>
                <button onClick={() => router.back()} className="text-[10px] font-extrabold text-school-primary uppercase tracking-widest hover:underline">
        ...
        <div className="min-h-screen bg-background text-foreground animate-in fade-in duration-700">
            <div className="max-w-7xl mx-auto p-4 md:p-8 lg:p-12 space-y-10">
                ...
                <button 
                    ...
                    className="inline-flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-muted-foreground hover:text-foreground transition-all"
                >
                    <ArrowLeft className="h-4 w-4" /> Back to Registry Hub
                </button>
                                ...
                                <div className="space-y-2 text-center md:text-left">
                                    <div className="flex flex-wrap items-center justify-center md:justify-start gap-3">
                                        ...
                                        <div className="flex items-center gap-1.5 px-3 py-1 bg-school-primary-50 rounded-lg border border-school-primary-200">
                                            <Users className="h-3 w-3 text-school-primary" />
                                            <span className="text-[9px] font-bold text-school-primary uppercase tracking-widest">Student Profile</span>
                                        </div>
                                    </div>
                                    <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 pt-2">
                                        <div className="flex items-center gap-1.5 text-muted-foreground text-xs font-semibold uppercase tracking-tight">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                    <div className="lg:col-span-2 space-y-12">
                        {/* Placement Registry */}
                        <Section title="Hub Placement" icon={<BookOpen className="h-5 w-5" />}>
                            {user.assignedClasses.length === 0 ? <EmptySection message="No institutional classroom placement discovered." /> : (
                                <div className="grid gap-4 grid-cols-1 sm:grid-cols-2">
                                    {user.assignedClasses.map(c => (
                        ...
                        {/* Subject Allocations */}
                        <Section title="Academic Hub Allocations" icon={<ShieldCheck className="h-5 w-5" />}>
                            {user.allocatedSubjects.length === 0 ? <EmptySection message="No subject modules currently synchronized." /> : (
                                <div className="flex flex-wrap gap-3">
                                    {user.allocatedSubjects.map(sub => (
                        ...
                        {/* Telemetry */}
                        <Section title="Performance Telemetry" icon={<ClipboardCheck className="h-5 w-5" />}>
                            {user.assessments.length === 0 ? <EmptySection message="No hub performance records synchronized." /> : (
                                <div className="space-y-3">
                                    {user.assessments.map(a => {
                                        const pct = a.maxScore && a.score !== null ? Math.round((a.score / a.maxScore) * 100) : null;
                                        const colorClass = pct === null ? 'text-muted-foreground' : pct >= 70 ? 'text-emerald-500' : pct >= 50 ? 'text-amber-500' : 'text-destructive';
                                        ...
                                        return (
                                            <div key={a.id} className="flex items-center justify-between gap-4 p-5 rounded-[1.5rem] bg-card border border-border hover:bg-surface transition-all shadow-sm group">
                                                <div className="min-w-0 flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        <p className="text-sm font-extrabold text-foreground uppercase italic truncate">{a.subject ?? 'External Hub'}</p>
                                                ...
                                                <div className="text-right shrink-0">
                                                    <p className={cn("text-base font-extrabold italic tracking-tighter tabular-nums", colorClass)}>
                                                        {a.score !== null ? a.score : '—'}{a.maxScore ? ` / ${a.maxScore}` : ''}
                                                    </p>
                                                    {pct !== null && <p className={cn("text-[10px] font-bold uppercase tracking-widest", colorClass)}>{pct}% Hub Sync</p>}
                                                </div>
                                            </div>
                                        );
                                    })}
                    ...
                    <div className="space-y-8">
                         <div className="p-8 rounded-[2rem] bg-surface border border-border space-y-4 shadow-inner">
                            ...
                            <p className="text-[11px] text-muted-foreground leading-relaxed italic font-medium">
                                Student identity profiles are institutional hub property. All performance telemetry is cryptographically synchronized with the master syllabus registry.
                            </p>
                        </div>
                    </div>
                </div>
            ...
            {showAssign && user.schoolId && (
                <AssignClassModal 
                    ...
                    onSuccess={() => { 
                        setShowAssign(false); 
                        getUserById(id, actorProfile?.schoolId ?? null).then(setUser); 
