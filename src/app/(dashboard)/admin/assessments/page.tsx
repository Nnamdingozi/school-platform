...
import { getAdminExamRegistry } from '@/app/actions/exam-engine.actions';
import { checkSubscription } from "@/app/actions/subscription-guard";
import { ExamRegistryClient } from "@/components/admin-dasboard/examRegistryClient";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  if (!user) return { title: "Exam Registry | SchoolPaaS" };
  ...
  return {
    title: `Exam Registry | ${profile?.school?.name || "Institution"} | SchoolPaaS`,
    description: "Institutional oversight of all active and completed CBT sessions."
  };
}
...
export default async function Page() {
  // 1. Resolve Identity & Subscription (Rule 10 & 11)
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  ...
  // Non-admins and Independent Learners are blocked from the Registry.
  ...
  // 2. Fetch Tier-2 Institutional Data
  const result = await getAdminExamRegistry(profile.schoolId);

  return (
    <ExamRegistryClient 
      initialStats={result.success ? result.stats! : null} 
      initialExams={result.success ? result.exams! : []} 
    />
  );
}