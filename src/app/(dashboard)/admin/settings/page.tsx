...
export async function generateMetadata(): Promise<Metadata> {
    ...
    if (!user) return { title: "Settings | SchoolPaaS" };
    ...
    return {
        title: `Settings | ${profile?.school?.name || "Personal Registry"} | SchoolPaaS`,
        description: "Configure institutional identity and system-wide academic logic."
    };
}
...
export default async function SettingsPage() {
    ...
    // Parallel fetch: settings (for schools) or null (for individuals)
    const settingsData = profile.schoolId 
        ? await getSchoolSettings(profile.schoolId)
        : null;

    return (
        <SettingsClient 
            initialData={settingsData} 
            isIndependent={!profile.schoolId || profile.role === Role.INDIVIDUAL_LEARNER}
        />
    );
}