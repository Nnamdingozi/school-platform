...
import { getStudentClassRegistry } from "@/app/actions/student-class";
import { ClassRegistryClient } from "@/components/student-dashboard/class/classRegistryClient";
import { Role } from "@prisma/client";
...
export async function generateMetadata(): Promise<Metadata> {
    ...
    if (!user) return { title: "Class Registry | SchoolPaaS" };
    ...
    return {
        title: `Class Registry | ${profile?.school?.name || "Registry"} | SchoolPaaS`,
    };
}
...
export default async function Page() {
    ...
    // Rule 6: Identity Context
    const isIndependent = !profile.schoolId || profile.role === Role.INDIVIDUAL_LEARNER;

    // Rule 11: Fetch Registry Truth (Returns null for independent learners automatically)
    const registryData = await getStudentClassRegistry(profile.id, profile.schoolId);
