...
import { getClassSubjectAllocation } from "@/app/actions/subject-allocation";
import { StudentElectiveClient } from "@/components/subjects/subject-elective";
...
export async function generateMetadata(): Promise<Metadata> {
    ...
    if (!user) return { title: "Electives | SchoolPaaS" };

    return {
        title: "Subject Selection | Academic Registry | SchoolPaaS",
        description: "Customize your academic path by selecting your elective courses."
    };
}
...
export default async function Page() {
    ...
    const profile = await prisma.profile.findUnique({
        ...
        include: { 
            classEnrollments: { take: 1 },
            studentSubjects: { select: { gradeSubjectId: true } }
        }
    });

    if (!profile || profile.role !== Role.STUDENT) redirect("/login");

    const enrollment = profile.classEnrollments[0];
    
    // Rule 5: Students must be placed in a class to select electives
    if (!enrollment?.classId || !profile.schoolId) {
        redirect("/student?error=placement_required");
    }

    // 2. Fetch Institutional Logic (Rule 11)
    const allocationData = await getClassSubjectAllocation(enrollment.classId, profile.schoolId);

    if (!allocationData) {
        return (
            <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6">
                <p className="text-slate-500 uppercase font-black italic tracking-widest">Academic Registry Offline</p>
            </div>
        );
    }

    // 3. Resolve existing selections
    const initialSelectedIds = profile.studentSubjects.map(s => s.gradeSubjectId);

    return (
        <StudentElectiveClient 
