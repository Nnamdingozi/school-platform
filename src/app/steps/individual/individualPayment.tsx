...
// import React, { useState, useEffect, useCallback } from 'react';
// import { useRegisterStore } from '@/store/individualOnboardingStore';
// import { completeIndividualOnboarding, verifyIndividualPayment } from '@/app/actions/individualOnboarding';
// import { type SubscriptionPlanItem } from '@/app/actions/subscription.actions';
// import { type PlanType } from '@/store/onboardingStore';
// import { 
//     Check, ArrowLeft, Loader2, CreditCard, 
...
// import React, { useState, useEffect, useCallback } from 'react';
// import { useRegisterStore } from '@/store/individualOnboardingStore';
// import { verifyIndividualPayment, completeIndividualOnboarding } from '@/app/actions/individualOnboarding';
// import { type SubscriptionPlanItem } from '@/app/actions/subscription.actions';
// import { Check, ArrowLeft, Loader2, CreditCard, Zap, Lock, ShieldCheck, ChevronRight } from 'lucide-react';
// import { cn } from '@/lib/utils';
// import { toast } from 'sonner';
// import { Button } from '@/components/ui/button';
// import { getErrorMessage } from '@/lib/error-handler';
...
//         const planSlug = activePlan.slug ?? 'individual-monthly';
//         const amount = currency === 'ngn'
//             ? activePlan.priceKobo
//             : activePlan.priceUSD * 100;
...
//                             const verification = await verifyIndividualPayment(response.reference);
//                             if (!verification.success) {
//                                 throw new Error('Payment verification failed. Contact support if debited.');
//                             }

//                             // 2. Provision subscription in DB
...
//                             // 3. Lock store — user can never reach payment screen again
//                             setPaymentSuccess(response.reference, planSlug);
//                             setComplete(true);
//                             toast.success('Personal Registry Synchronized.');
//                         } catch (err: unknown) {
//                             toast.error(getErrorMessage(err));
//                             // Reset loading states so user can retry if not debited,
//                             // but store remains unlocked so hasPaid guard still works
//                             // if setPaymentSuccess already fired before the error.
//                             setIsProcessing(false);
//                             setIsGateOpening(false);
//                         }
//                     };
...
//         } catch (err: unknown) {
//             setIsGateOpening(false);
//             console.error('[PAYMENT_GATEWAY_FAULT]:', getErrorMessage(err));
//             toast.error('Gateway connection failed. Please try again.');
//         }
//     }, [
//         activePlan, currency, email, name,
//         isGateOpening, isProcessing, hasPaid,
//         setPaymentSuccess, setComplete,
//     ]);
...
//                 </div>
//                 <div className="space-y-2">
//                     <h3 className="text-xl font-extrabold text-foreground uppercase italic tracking-tight">
...
//                                     <div className="flex items-center gap-3">
//                                         <Zap className={cn('h-4 w-4', isSelected ? 'text-school-primary' : 'text-muted-foreground/40')} />
//                                         <p className="text-lg font-extrabold text-foreground uppercase italic tracking-tighter leading-none">
//                                             {p.name}
//                                         </p>
//                                     </div>
//                                     <ul className="space-y-1.5">
...
// ── NO declare global here — PaystackPop is already declared in payment-step.tsx
//    TypeScript merges all declare global blocks globally, so re-declaring
//    causes a type conflict. The school file's declaration covers both files.
    ...
    const handlePayment = useCallback(() => {
        ...
        try {
            const handler = window.PaystackPop.setup({
                ...
                amount,
                currency: currency.toUpperCase(),
                // Match the global PaystackSetupConfig metadata shape from payment-step.tsx
                metadata: {
                    plan: planSlug,
                    schoolName: 'Individual Learner',  // required by global type, neutral value
                    adminName: name,
                },
                callback: (response) => {
                    setIsProcessing(true);
