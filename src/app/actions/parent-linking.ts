...
/**
 * Links a specific parent to a specific student within a school.
...
async function linkSinglePair(
  ...
  try {
    // 2. Fetch profiles - Rule 5: Must match schoolId
    const [parent, student] = await Promise.all([
      prisma.profile.findFirst({ where: { email: trimmedParent, schoolId } }),
      prisma.profile.findFirst({ where: { email: trimmedChild, schoolId } }),
    ]);
    ...
    if (!parent) return { parentEmail, childEmail, success: false, message: 'Parent account not found in this school.' };
    if (parent.role !== Role.PARENT) return { parentEmail, childEmail, success: false, message: 'Target user is not a Parent.' };
    
    if (!student) return { parentEmail, childEmail, success: false, message: 'Student account not found in this school.' };
    if (student.role !== Role.STUDENT) return { parentEmail, childEmail, success: false, message: 'Target user is not a Student.' };

    if (parent.id === student.id) return { parentEmail, childEmail, success: false, message: 'Self-linking is forbidden.' };

    // 3. Create Link - Using the correct compound unique key from schema
    await prisma.parentStudent.upsert({
      where: {
    ...
    await logActivity({
        ...
        title: "Family Link Created",
        description: `Linked Parent (${parent.name ?? parent.email}) to Student (${student.name ?? student.email})`
    });
    ...
    return {
      ...
      childEmail,
      success: true,
      message: `Successfully linked ${student.name ?? student.email} to ${parent.name ?? parent.email}.`,
    };
  } catch (error: unknown) {
    return {
      parentEmail,
...
/**
 * Bulk action for linking parents to children
 */
export async function bulkLinkParentsAndChildren(
  input: ParentChildLinkInput[],
  schoolId: string,
  ...
  for (const row of input) {
    const result = await linkSinglePair(row.parentEmail, row.childEmail, schoolId, actorId, actorRole);
    results.push(result);
  }
...
/**
 * Directory: Paginated and Searchable Family View
 * Rule 5: Scoped to schoolId
 */
export async function getLinkedFamiliesPaginated(
  schoolId: string, 
  page: number = 1, 
    ...
    const [families, total] = await Promise.all([
      prisma.profile.findMany({
        ...
        select: {
          ...
          name: true,
          email: true,
          // Relation name from your Prisma Schema
          ParentStudent_ParentStudent_parentIdToprofiles: {
            select: {
              id: true,
...
/**
 * Action to remove a specific link
 * Rule 10: Ensures linkage belongs to the school
 */
export async function unlinkParentStudent(
    ...
    try {
        ...
        if (!link) throw new Error("Linkage not found in this institution.");
        ...
        await logActivity({
            ...
            actorRole,
            type: ActivityType.SETTINGS_UPDATED,
            title: "Family Link Removed",
            description: `Deleted relationship link ID: ${linkId}`
        });
