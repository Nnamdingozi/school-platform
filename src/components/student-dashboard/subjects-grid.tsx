...
//             Registry Ledger Empty: Provision modules to your learning path to begin.
//           </p>
//         </div>
//       ) : (
...
//                 <div 
//                     className="h-full bg-school-primary transition-all duration-1000 ease-out" 
 ...
 * ACADEMIC MODULE MATRIX (Tier 2/3)
 * Rule 11: High-fidelity Registry Typography (font-extrabold italic).
 * Rule 15: Zero 'any' types in props or internal logic.
 * Rule 18: Semantic Flip (bg-background, bg-card, bg-surface).
 * Rule 19: Standardized Geometry [2rem].
...
export function SubjectsGrid({
  ...
  /**
   * Rule 23: Logical Hub Protection
   */
  const handleSelection = (subject: GradeSubjectHub | null) => {
    try {
      setSelectedSubject(subject);
    } catch (error: unknown) {
      const message = getErrorMessage(error);
      console.error(`[MODULE_SELECTION_FAULT]: ${message}`);
    }
  };
...
// ── Curriculum View Component (Rule 15/19/21) ────────────────────────────────
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-right-6 duration-700">
      <button 
        ...
        className="inline-flex items-center gap-2 text-muted-foreground hover:text-foreground uppercase font-extrabold text-[10px] tracking-widest transition-all group"
      >
        <ArrowLeft className="h-4 w-4 group-hover:-translate-x-1 transition-transform" /> Back to Hub Hub
      </button>
