...
import { Check, UserCircle, Save, Loader2, Info, GraduationCap } from 'lucide-react'
import { 
    getClassAllocationData, 
...
/**
 * ACADEMIC ALLOCATION MATRIX (Tier 2)
...
export function AllocationMatrix({ classId, schoolId }: { classId: string, schoolId: string }) {
    ...
    useEffect(() => {
        getClassAllocationData(classId, schoolId).then(res => {
            if (res.success && res.data) {
                ...
                const initialMap: MatrixState = {}
                res.data.existingMap.forEach((link) => {
                    // Rule 15 Fix: Ensure studentId is not null before indexing
                    if (link.studentId) {
                        if (!initialMap[link.studentId]) initialMap[link.studentId] = []
                        initialMap[link.studentId].push(link.gradeSubjectId)
                    }
    ...
    const handleSave = () => {
        startTransition(async () => {
            try {
                // ✅ RESOLVED TS2554: Passing a single object as expected by the action
                const savePromises = Object.entries(matrix).map(([studentId, gradeSubjectIds]) => 
                    allocateStudentSubjects({
                        schoolId,
                        studentId,
                ...
                if (errors.length > 0) {
                    toast.error(`${errors.length} identity nodes failed synchronization.`)
                } else {
                    toast.success("Academic matrix synchronized successfully.")
                }
            } catch (err) {
    ...
    if (loading) return (
        <div className="p-20 flex flex-col items-center justify-center gap-6 animate-pulse">
            ...
            <p className="text-muted-foreground font-bold text-[10px] uppercase tracking-[0.3em]">Querying_Academic_Matrix...</p>
        </div>
    )
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-700">
            {/* ── HEADER ── */}
            <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-card border border-border p-6 md:p-10 rounded-[2rem] shadow-xl">
                <div className="flex items-center gap-6">
                    <div className="h-14 w-14 rounded-2xl bg-school-primary-50 border border-school-primary-200 flex items-center justify-center shadow-inner">
                    ...
                    <div>
                        ...
                        <p className="text-school-primary text-[10px] font-bold uppercase tracking-widest mt-2">
                            {data.gradeName} • Level {data.gradeLevel} Registry
                        </p>
                    </div>
                </div>
                <button 
                    onClick={handleSave} 
                    disabled={isPending}
                    className="w-full md:w-auto bg-school-primary text-on-school-primary font-extrabold px-10 py-5 rounded-2xl hover:brightness-110 active:scale-95 transition-all flex items-center justify-center gap-3 shadow-lg shadow-school-primary-200 disabled:opacity-30"
                >
                    {isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}
                    Sync Allocations
                </button>
            ...
            {/* WAEC Minimum Info for Senior Secondary */}
            {data.gradeLevel >= 10 && (
                <div className="bg-amber-500-50 border border-amber-200 p-6 rounded-2xl flex items-start gap-4">
                    <Info className="h-5 w-5 text-amber-600 shrink-0 mt-0.5" />
                    <div className="space-y-1">
                        <p className="text-[10px] font-extrabold text-amber-600 uppercase tracking-widest">Academic Policy Protocol</p>
                        <p className="text-xs text-foreground/70 leading-relaxed font-medium italic">
                            Senior Level node detected. Enforcing <span className="text-foreground font-bold">minimum 9 subjects</span> per identity to comply with WAEC/NECO standards.
                        </p>
                    </div>
                </div>
            )}
            ...
            <div className="rounded-[2rem] border border-border bg-card overflow-hidden shadow-2xl relative">
                {isPending && (
                    <div className="absolute inset-0 bg-background/60 backdrop-blur-[2px] z-50 flex items-center justify-center">
                        <span className="font-extrabold text-school-primary text-[10px] uppercase tracking-[0.4em] animate-pulse">Processing_Registry_Updates...</span>
                    </div>
                )}
                ...
                <div className="overflow-x-auto custom-scrollbar">
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-surface/80 backdrop-blur-md text-[10px] font-bold uppercase tracking-widest text-muted-foreground border-b border-border">
                                <th className="p-6 md:p-8 sticky left-0 bg-surface z-20 min-w-[320px] shadow-xl">Student Identity Registry</th>
                                {data.subjects.map((sub) => (
                                    <th key={sub.id} className="p-6 md:p-8 text-center border-l border-border whitespace-nowrap min-w-[150px] italic">
                                        {sub.name}
                                    </th>
                                ))}
                            </tr>
                        ...
                        <tbody className="divide-y divide-border">
                            {data.students.map((student) => (
                                <tr key={student.id} className="hover:bg-muted/30 transition-all group">
                                    <td className="p-6 md:p-8 sticky left-0 bg-card z-10 group-hover:bg-surface transition-colors shadow-2xl flex items-center gap-4 border-r border-border">
                                        <div className="h-10 w-10 rounded-xl bg-surface border border-border flex items-center justify-center text-muted-foreground group-hover:text-school-primary transition-colors shadow-inner">
                                            <UserCircle className="h-6 w-6" />
                                        </div>
                                        <div className="flex flex-col min-w-0">
                                            {/* ✅ RESOLVED TS2538: student.id is used as index only when confirmed string */}
                                            <span className="text-sm text-foreground font-extrabold uppercase tracking-tight truncate leading-none">{student.name || "UNNAMED"}</span>
                                            <span className="text-[10px] text-muted-foreground font-mono mt-1.5 opacity-60 truncate">{student.email}</span>
                                        </div>
                                    </td>
                                    {data.subjects.map((sub) => {
                                        const isChecked = student.id ? matrix[student.id]?.includes(sub.id) : false
                                        return (
                                            <td key={sub.id} className="p-6 md:p-8 text-center border-l border-border">
                                                <button 
                                                    onClick={() => student.id && toggle(student.id, sub.id)}
                                                    className={cn(
                                                        "h-9 w-9 rounded-xl border transition-all flex items-center justify-center mx-auto",
                                                        isChecked 
                                                            ? 'bg-school-primary border-school-primary text-on-school-primary scale-110 shadow-lg shadow-school-primary-200' 
                                                            : 'bg-surface border-border text-muted-foreground/20 hover:border-school-primary/40'
                                                    )}
                                                >
                                                    {isChecked && <Check className="h-5 w-5 stroke-[4]" />}
                                                </button>
