...
import { Avatar, AvatarFallback } from '@/components/ui/avatar'
import { type UnassignedStudentsData } from '@/app/actions/analytics.action'
import { useRouter } from 'next/navigation'

/**
 * ✅ FIX: Interface defined to ensure property 'initialData' exists on type.
 * Rule 15: Strictly typed props matching Server Action response.
 */
...
export function UnassignedStudentsAlert({ initialData }: UnassignedStudentsAlertProps) {
    ...
    // Rule 11: Display "Success" state if no data or all students assigned
    if (!initialData || remainingCount === 0) return (
        <Card className="bg-slate-900 border-white/5 rounded-[2.5rem] p-10 flex flex-col items-center justify-center text-center space-y-4">
            ...
            <h3 className="text-xl font-black text-white uppercase italic">Placement Optimal</h3>
            <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest italic">
                All provisioned student identities have been synchronized with classroom registries.
            </p>
        </Card>
    );
    ...
    return (
        <Card className="bg-slate-900 border-white/5 rounded-[2.5rem] overflow-hidden shadow-2xl">
            <CardHeader className="p-8 bg-slate-950/40 border-b border-white/5 flex flex-row items-center justify-between">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <CardTitle className="text-sm font-black uppercase italic text-white">Registry Outliers</CardTitle>
                        <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">
                            {remainingCount} Students Pending Institutional Placement
                        </p>
                    </div>
                </div>
                <button 
                    onClick={() => router.push('/admin/users?tab=unassigned')}
                    className="text-[9px] font-black text-school-primary uppercase tracking-[0.2em] flex items-center gap-2 hover:brightness-125 transition-all"
            ...
            <CardContent className="p-8 space-y-4">
                {visibleStudents.map(student => (
                    <div key={student.id} className="flex items-center justify-between p-4 bg-slate-950 rounded-2xl border border-white/5 hover:border-white/10 transition-all group">
                        <div className="flex items-center gap-4">
                            <Avatar className="h-10 w-10 border border-white/5">
                                <AvatarFallback className="bg-slate-900 font-black text-xs uppercase" style={{ color: primaryColor }}>
                                    {(student.name || student.email).substring(0, 2).toUpperCase()}
                                </AvatarFallback>
                            </Avatar>
                            <div>
                                <p className="text-sm font-bold text-white uppercase italic">{student.name || "Anonymous Learner"}</p>
                                <p className="text-[9px] text-slate-600 font-bold uppercase tracking-tighter">{student.email}</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-2">
                            <button 
