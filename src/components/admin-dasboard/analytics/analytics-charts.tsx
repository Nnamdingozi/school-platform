...
/**
 * Rule 17: Charts use primary color shades for visual consistency.
 */
function useChartBranding() {
    const { profile } = useProfileStore();
    const base = profile?.primaryColor || "#f59e0b";
    return [base, `${base}CC`, `${base}99`, `${base}66`, `${base}33` ];
}

export function GradeDistributionChart({ data }: { data: any[] }) {
    const colors = useChartBranding();
    return (
        <Card className="bg-slate-900 border-white/5 rounded-[2rem] shadow-xl overflow-hidden">
            <CardHeader className="p-8 border-b border-white/5 bg-slate-950/50">
                <CardTitle className="text-xs font-black uppercase tracking-widest text-white italic">Demographic Spread</CardTitle>
            </CardHeader>
...
export function AssessmentScoresChart({ data }: { data: any[] }) {
    const colors = useChartBranding();
    return (
        <Card className="bg-slate-900 border-white/5 rounded-[2rem] shadow-xl overflow-hidden">
            <CardHeader className="p-8 border-b border-white/5 bg-slate-950/50">
                <CardTitle className="text-xs font-black uppercase tracking-widest text-white italic">Subject Proficiency</CardTitle>
            </CardHeader>
            <CardContent className="p-8 h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={data}>
...
export function CommunicationTrendChart({ data }: { data: any[] }) {
    const colors = useChartBranding();
    return (
        <Card className="bg-slate-900 border-white/5 rounded-[2rem] shadow-xl overflow-hidden">
            <CardHeader className="p-8 border-b border-white/5 bg-slate-950/50">
                <CardTitle className="text-xs font-black uppercase tracking-widest text-white italic">Transmission Velocity</CardTitle>
            </CardHeader>
            <CardContent className="p-8 h-[250px]">
                <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={data}>
                        ...
                        <Tooltip />
                        <Area type="monotone" dataKey="count" stroke={colors[0]} fill="url(#areaColor)" strokeWidth={3} />
                    </AreaChart>
                </ResponsiveContainer>
            </CardContent>
        </Card>
    ...
    return (
        <Card className="bg-slate-900 border-white/5 rounded-[2.5rem] overflow-hidden shadow-2xl">
            <CardHeader className="p-8 bg-slate-950/50 border-b border-white/5">
                <CardTitle className="text-xs font-black uppercase text-white italic tracking-widest">Enrollment Status</CardTitle>
            </CardHeader>
                ...
                <div className="grid grid-cols-2 gap-4">
                    {data.map((item, i) => (
                        <div key={i} className="bg-slate-950 p-3 rounded-xl border border-white/5">
                            <p className="text-[8px] font-black uppercase text-slate-600 mb-1">{item.name}</p>
                            <p className="text-lg font-bold text-white italic leading-none" style={{ color: colors[i % colors.length] }}>{item.value}</p>
                        </div>
                    ))}
                </div>
            </CardContent>
...
export function AcademicContentHealth({ data }: { data: any }) {
    const { profile } = useProfileStore();
    const primaryColor = profile?.primaryColor || "#f59e0b";
    
    const lessonPct = data.totalTopics > 0 ? Math.round((data.withLessons / data.totalTopics) * 100) : 0
    const quizPct = data.withLessons > 0 ? Math.round((data.withQuizzes / data.withLessons) * 100) : 0

    return (
        <Card className="bg-slate-900 border-white/5 rounded-[2.5rem] overflow-hidden shadow-2xl">
            <CardHeader className="p-8 bg-slate-950/50 border-b border-white/5">
                <CardTitle className="text-xs font-black uppercase text-white italic tracking-widest">Curriculum Health</CardTitle>
            </CardHeader>
            <CardContent className="p-8 space-y-8">
                <ProgressItem label="AI Lesson Synthesis" pct={lessonPct} color={primaryColor} />
                <ProgressItem label="Quiz Registry Coverage" pct={quizPct} color={`${primaryColor}99`} />
                <p className="text-[9px] text-slate-600 font-bold uppercase tracking-tighter italic">Percentage of topics with active academic nodes.</p>
            </CardContent>
        </Card>
    )
}

function ProgressItem({ label, pct, color }: { label: string, pct: number, color: string }) {
    return (
