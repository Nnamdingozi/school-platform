...
// /**
//  * FAMILY REGISTRY DIRECTORY (Tier 2)
...
//         toast.error("Failed to synchronize family ledger.");
//     } finally {
//         setLoading(false);
//     }
//   }, [schoolId, page, search]);
...
//   const handleUnlink = (linkId: string) => {
//     if (!confirm("Protocol Warning: Are you sure you want to decouple this relationship?")) return;
    
//     startTransition(async () => {
//         const res = await unlinkParentStudent(linkId);
//         if (res.success) {
//             toast.success("Relationship record purged.");
//             loadRegistry();
//         } else {
//             toast.error("Unlink protocol failed.");
//         }
//     });
//   };
...
//       {/* ── SEARCH & TELEMETRY HUB (Rule 18/19) ── */}
//       <div className="flex flex-col md:flex-row justify-between items-center gap-6 bg-surface border border-border p-4 md:p-6 rounded-2xl shadow-sm">
...
//             className={cn(
//                 "w-full bg-card border border-border rounded-xl pl-11 pr-4 py-3 text-sm text-foreground outline-none transition-all",
//                 "focus:ring-2 focus:ring-school-primary-200 focus:border-school-primary",
//                 "placeholder:text-muted-foreground/30 font-semibold"
//             )}
//             value={search}
//             onChange={(e) => setSearch(e.target.value)}
//           />
//         </div>
//         <div className="flex items-center gap-3 px-5 py-2.5 rounded-full bg-school-primary-50 border border-school-primary-200 shadow-inner">
//             <Activity className="h-3.5 w-3.5 text-school-primary" />
//             <span className="text-[10px] font-extrabold uppercase tracking-widest text-school-primary tabular-nums">
//                 {data.totalCount} Registered Families
//             </span>
//         </div>
//       </div>

//       {/* ── REGISTRY TABLE (Rule 19) ── */}
//       <div className="bg-card border border-border rounded-[2rem] overflow-hidden shadow-xl relative">
//         {isPending && (
...
//             <table className="w-full text-left border-collapse">
//             <thead>
//                 <tr className="bg-surface/50 text-[10px] font-bold uppercase tracking-widest text-muted-foreground border-b border-border">
//                 <th className="p-6 w-12"></th>
//                 <th className="p-6 italic">Guardian Identity</th>
//                 <th className="p-6 text-center italic">Dependent Count</th>
//                 <th className="p-6 text-right italic">Audit Hub</th>
//                 </tr>
//             </thead>
//             <tbody className="divide-y divide-border">
//                 {loading ? (
//                 <tr><td colSpan={4} className="p-24 text-center"><Loader2 className="animate-spin mx-auto text-school-primary h-8 w-8" /></td></tr>
//                 ) : data.families.length === 0 ? (
//                 <tr><td colSpan={4} className="p-24 text-center text-muted-foreground text-[10px] font-bold uppercase tracking-widest italic opacity-40">No identity records discovered.</td></tr>
//                 ) : data.families.map((parent) => (
//                 <React.Fragment key={parent.id}>
//                     <tr 
//                     onClick={() => toggleRow(parent.id)}
...
//                     >
//                     <td className="p-6">
//                         <div className="h-8 w-8 rounded-lg bg-surface border border-border flex items-center justify-center transition-colors group-hover:border-school-primary-200">
//                             {expandedRows.has(parent.id) 
//                                 ? <ChevronDown className="h-4 w-4 text-school-primary" /> 
...
//                         <div className="h-10 w-10 rounded-xl bg-surface border border-border flex items-center justify-center text-muted-foreground/30 group-hover:bg-school-primary-50 group-hover:text-school-primary transition-all shadow-inner">
//                             <User className="h-5 w-5" />
//                         </div>
//                         <div className="min-w-0">
//                             <p className="text-sm font-extrabold text-foreground uppercase italic tracking-tight truncate leading-none">
//                                 {parent.name || "Anonymous_Identity"}
//                             </p>
//                             <p className="text-[10px] text-muted-foreground font-mono mt-1.5 opacity-60 truncate">
//                                 {parent.email}
//                             </p>
//                         </div>
//                         </div>
...
//                         <span className="bg-school-primary-50 text-school-primary text-[10px] font-extrabold px-3 py-1.5 rounded-full border border-school-primary-200 shadow-sm tabular-nums">
//                         {parent.ParentStudent_ParentStudent_parentIdToprofiles.length} Hubs
//                         </span>
//                     </td>
//                     <td className="p-6 text-right">
//                         <button className="text-[10px] font-extrabold text-muted-foreground hover:text-school-primary uppercase tracking-widest transition-all px-4 py-2 rounded-lg hover:bg-surface border border-transparent hover:border-border">
//                             Profile Hub
//                         </button>
//                     </td>
//                     </tr>

//                     {/* ── EXPANDED DEPENDENTS HUB ── */}
//                     {expandedRows.has(parent.id) && (
//                     <tr className="bg-surface/30">
//                         <td colSpan={4} className="p-0">
//                         <div className="px-16 py-6 space-y-3 animate-in slide-in-from-top-2 duration-300">
//                             <div className="flex items-center gap-2 mb-4">
//                                 <Activity className="h-3 w-3 text-school-primary/40" />
//                                 <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-[0.2em]">Linked Student Profiles</p>
//                             </div>
//                             {parent.ParentStudent_ParentStudent_parentIdToprofiles.map((link) => (
//                             <div key={link.id} className="flex items-center justify-between p-4 rounded-2xl bg-card border border-border hover:border-school-primary-200 transition-all shadow-sm group/link">
//                                 <div className="flex items-center gap-4 min-w-0">
//                                 <div className="h-8 w-8 rounded-lg bg-surface border border-border flex items-center justify-center">
//                                     <GraduationCap className="h-4 w-4 text-school-primary opacity-60" />
//                                 </div>
//                                 <div className="min-w-0">
//                                     <p className="text-xs font-extrabold text-foreground uppercase italic tracking-tight truncate">{link.student.name || "Anonymous_Node"}</p>
//                                     <p className="text-[9px] text-muted-foreground font-mono mt-0.5 opacity-60">{link.student.email}</p>
//                                 </div>
//                                 </div>
//                                 <button 
//                                 onClick={(e) => {
...
//                                 }}
//                                 disabled={isPending}
//                                 className="p-2.5 rounded-xl bg-surface border border-border text-muted-foreground hover:text-destructive hover:border-destructive/30 transition-all active:scale-90 shadow-sm"
//                                 title="Purge relationship record"
//                                 >
...
//       {/* ── PAGINATION TERMINAL (Rule 18/21) ── */}
//       <div className="flex items-center justify-between px-6 py-4 bg-surface border border-border rounded-2xl shadow-inner">
//         <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest italic">
//             Page <span className="text-foreground font-extrabold tabular-nums">{page}</span> of <span className="text-foreground font-extrabold tabular-nums">{data.totalPages || 1}</span>
//         </p>
//         <div className="flex gap-2">
//           <button 
//             disabled={page === 1 || loading}
//             onClick={() => setPage(p => p - 1)}
//             className="h-10 w-10 flex items-center justify-center rounded-xl bg-card border border-border text-muted-foreground disabled:opacity-20 hover:text-school-primary hover:border-school-primary-200 transition-all shadow-sm active:scale-90"
//           >
 ...
 * FAMILY REGISTRY DIRECTORY (Tier 2)
 * Rule 11: High-fidelity Registry Typography (font-extrabold italic).
 * Rule 15: Resolved TS2554 by providing full security context to the unlink action.
 * Rule 17: Leverages useProfileStore to prevent prop drilling of actor metadata.
 * Rule 18/19: Semantic Flip and [2rem] Geometry.
...
export function FamilyDirectoryTable({ schoolId }: { schoolId: string }) {
  ...
  /**
   * ✅ RESOLVED TS2554: Passing mandatory security context
   * linkId: Target relationship
   * schoolId: Institutional scope
   * actorId: Current Admin UUID
   * actorRole: Current Admin Authorization Tier
   */
  const handleUnlink = (linkId: string) => {
    if (!profile) return toast.error("Identity context not discovered.");
    if (!confirm("Protocol Warning: Are you sure you want to decouple this relationship?")) return;
    
    startTransition(async () => {
        const res = await unlinkParentStudent(
            linkId, 
            schoolId, 
            profile.id, 
        ...
        if (res.success) {
            ...
            toast.error(res.error || "Unlink protocol failed.");
        }
    });
  };
