...
export function ParentDashboardClient({
    ...
    const overallStats = useMemo(() => {
        ...
        const totalAssessments = currentSubjects.reduce((sum, s) => sum + s.assessments.length, 0)
        ...
        currentSubjects.forEach((subject) => {
            subject.topics.forEach((topic) => {
                if (topic.hasLesson) {
                    lessonsAvailable += 1
                    if (!topic.assessment) pendingAssessments += 1
                }
            })
        })
        ...
        return { totalSubjects, totalAssessments, lessonsAvailable, pendingAssessments, overallPct }
    }, [currentSubjects])

    const perSubjectAverages = useMemo(
        () =>
            currentSubjects.map((s) => {
                const valid = s.assessments.map((a) => a.pct).filter((p): p is number => p != null)
                const avg   = valid.length > 0 ? valid.reduce((sum, v) => sum + v, 0) / valid.length : 0
                return { id: s.gradeSubjectId, name: s.subjectName, avg }
            }),
        [currentSubjects]
    )
    ...
    return (
        <div
            ...
            style={{ '--color-primary': primaryColor, '--color-secondary': secondaryColor } as CSSProperties}
        >
            {/* ── Top nav ── */}
                ...
                <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="text-right">
                            <div className="text-sm font-medium">{parentName ?? 'Parent'}</div>
                        </div>
                        ...
                        >
                            ...
                            <svg className="h-5 w-5 text-slate-200" viewBox="0 0 24 24" fill="none">
                                <path d="M14 18.5a2 2 0 1 1-4 0" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" />
                                <path d="M6.5 10.5A5.5 5.5 0 1 1 18 10.5c0 1.97.74 3.06 1.34 3.87.64.88.96 1.32.9 1.66-.07.37-.37.67-.97 1.27-.9.9-1.36 1.35-1.9 1.56-.48.18-1.02.18-2.1.18H9.13c-1.08 0-1.62 0-2.1-.18-.54-.21-1-.66-1.9-1.56-.6-.6-.9-.9-.97-1.27-.06-.34.26-.78.9-1.66.6-.81 1.34-1.9 1.34-3.87Z" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" />
                            </svg>
                            {unreadCount > 0 && (
                    ...
                    <section className="space-y-4">
                        ...
                        <div className="grid gap-4 md:grid-cols-[minmax(0,2fr)_minmax(0,3fr)]">
                            ...
                            <div className="rounded-2xl border border-slate-800 bg-slate-900/80 p-4">
                                <div className="flex items-start justify-between gap-3">
                                    <div>
                                        <div className="text-xs uppercase tracking-wide text-slate-400">Student overview</div>
                                    ...
                                    <div className="flex flex-col items-center gap-1">
                                        <div className="relative h-20 w-20">
                                            <svg viewBox="0 0 48 48" className="h-20 w-20">
                                                <circle cx="24" cy="24" r="18" className="stroke-slate-800" strokeWidth="6" fill="none" />
                                                <circle
                                                    cx="24" cy="24" r="18"
                                                    className="stroke-primary"
                                                    strokeWidth="6" fill="none"
                                                    strokeDasharray={2 * Math.PI * 18}
                                                    strokeDashoffset={2 * Math.PI * 18 - percentageToStroke(overallStats.overallPct)}
                                                    strokeLinecap="round"
                            ...
                            <div className="grid gap-3 sm:grid-cols-2">
                                {[
                                    { label: 'Total subjects',      value: overallStats.totalSubjects,      color: 'text-slate-50'  },
                                    { label: 'Total assessments',   value: overallStats.totalAssessments,   color: 'text-slate-50'  },
                                    { label: 'Lessons available',   value: overallStats.lessonsAvailable,   color: 'text-slate-50'  },
                                    { label: 'Pending assessments', value: overallStats.pendingAssessments, color: 'text-amber-400' },
                                ].map((card) => (
                                    <div key={card.label} className="rounded-2xl border border-slate-800 bg-slate-900/80 p-3">
                                        <div className="text-[10px] uppercase tracking-wide text-slate-400">{card.label}</div>
                        ...
                        <div className="mt-2 border-b border-slate-800">
                            <div className="flex gap-4 text-xs">
                                {(['overview', 'subjects', 'assessments'] as const).map((key) => (
                                    <button
                                        key={key}
                                        type="button"
                                        onClick={() => setTab(key)}
                                        className={`relative py-2 capitalize ${
                                            tab === key ? 'text-primary' : 'text-slate-400 hover:text-slate-200'
                                        }`}
                                    >
                                        {key}
                                        {tab === key && (
                        ...
                        {tab === 'overview' && (
                            <div className="space-y-4 pt-2">
                                <h2 className="text-xs font-semibold text-slate-300">Subject progress</h2>
                                <div className="space-y-3">
                                    {currentSubjects.map((subject) => {
                                        const totalTopics      = subject.topics.length
                                        const topicsWithLessons = subject.topics.filter((t) => t.hasLesson).length
                                        const pct = totalTopics > 0
                                            ? Math.round((topicsWithLessons / totalTopics) * 100)
                                            : 0
                                        return (
                                            <div key={subject.gradeSubjectId} className="rounded-xl border border-slate-800 bg-slate-900/80 p-3">
                                                <div className="flex items-center justify-between gap-3">
                                                    <div>
                                                        <div className="text-xs font-medium text-slate-100">{subject.subjectName}</div>
                                                    ...
                                                    <div className="w-32">
                                                        ...
                                                        <div className="mt-1 text-right text-[10px] text-slate-400">{pct}% complete</div>
                                                    </div>
                                                </div>
                                            </div>
                                        )
                                    ...
                                    {currentSubjects.length === 0 && (
                                        <p className="text-xs text-slate-500">No subject enrollments found for this student yet.</p>
                                    )}
                                </div>
                            </div>
                        )}
                            ...
                            <div className="space-y-4 pt-2">
                                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                                    {currentSubjects.map((subject) => {
                                        ...
                                        return (
                                            <div key={id} className="rounded-2xl border border-slate-800 bg-slate-900/80 p-3">
                                                <button
                                                    type="button"
                                                    onClick={() => setExpandedSubjectIds((prev) => ({ ...prev, [id]: !isExpanded }))}
                                                    className="flex w-full items-center justify-between gap-3"
                                                >
                                                    <div className="text-left">
                                                        <div className="text-xs font-semibold text-slate-50">{subject.subjectName}</div>
                                                        <div className="mt-0.5 text-[11px] text-slate-400">
                                                            Teacher: {subject.teacherName ?? 'Not assigned yet'}
                                                        </div>
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        <div className="relative h-12 w-12">
                                                            <svg viewBox="0 0 48 48" className="h-12 w-12">
                                                                <circle cx="24" cy="24" r="18" className="stroke-slate-800" strokeWidth="5" fill="none" />
                                                                <circle
                                                                    cx="24" cy="24" r="18"
                                                                    className="stroke-primary"
                                                                    strokeWidth="5" fill="none"
                                                                    strokeDasharray={2 * Math.PI * 18}
                                                                    strokeDashoffset={2 * Math.PI * 18 - percentageToStroke(avg)}
                                                                    strokeLinecap="round"
                                                                    transform="rotate(-90 24 24)"
                                                                />
                                                            </svg>
                                                            <div className="absolute inset-0 flex flex-col items-center justify-center">
                                                                <span className="text-[11px] font-semibold text-slate-50">{Math.round(avg)}%</span>
                                                                <span className="text-[9px] text-slate-400">{letterFromPercentage(avg)}</span>
                                                            </div>
                                                        </div>
                                                        <span className="text-xs text-slate-400">{isExpanded ? 'Hide' : 'Details'}</span>
                                                    </div>
                                                </button>
                                                        ...
                                                        <div className="space-y-1 max-h-64 overflow-y-auto pr-1">
                                                            {subject.topics.map((topic: TopicStatus) => (
                                                                ...
                                                                >
                                                                    ...
                                                                    <span className="text-center text-slate-400">{topic.weekNumber ?? '-'}</span>
                                                                    <span className="text-center text-slate-400">{topic.termName}</span>
                        ...
                        {tab === 'assessments' && (
                            <div className="space-y-4 pt-2">
                                ...
                                <div className="rounded-2xl border border-slate-800 bg-slate-900/80 p-3">
                                    <div className="mb-2 text-xs font-semibold text-slate-200">Subject averages</div>
                                    <div className="flex flex-wrap gap-2">
                                        {perSubjectAverages.map((s) => (
                                            <div key={s.id} className="flex items-center gap-2 rounded-full border border-slate-800 bg-slate-950/60 px-3 py-1.5 text-[11px]">
                                                <span className="text-slate-200">{s.name}</span>
                                                <span className="h-1 w-14 overflow-hidden rounded-full bg-slate-800">
                                                    <span className="block h-1 rounded-full bg-primary" style={{ width: `${Math.round(s.avg)}%` }} />
                                                </span>
                                                <span className="text-slate-300">{Math.round(s.avg)}%</span>
                                        ...
                                        {perSubjectAverages.length === 0 && (
                                            <p className="text-xs text-slate-500">No assessments recorded yet.</p>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="space-y-2">
                                    {currentAssessments.map((a: AssessmentRecord) => (
                                        <article key={a.id} className="rounded-2xl border border-slate-800 bg-slate-900/80 p-3">
                                            <div className="flex flex-wrap items-center justify-between gap-3">
                                                <div>
                                                    <div className="text-xs font-semibold text-slate-50">{a.subjectName}</div>
                                                    <div className="text-[11px] text-slate-400">{a.topicTitle || 'General assessment'}</div>
                                                </div>
                                                <div className="flex items-center gap-2">
                                                    <span className="rounded-full bg-slate-800 px-2 py-0.5 text-[10px] text-slate-300">{a.type}</span>
                                            ...
                                            <div className="mt-2 flex flex-wrap items-center justify-between gap-3">
                                                <div className="flex items-baseline gap-2">
                                                    ...
                                                    {a.pct != null && (
                                                        <>
                                                            <span className="text-xs text-slate-400">({Math.round(a.pct)}%)</span>
                                                            <span className="rounded-full bg-slate-800 px-2 py-0.5 text-[10px] text-slate-300">
                                            ...
                                            {a.feedbacks.length > 0 && (
                                                <div className="mt-2 space-y-1">
                                                    {a.feedbacks.map((f, idx) => (
                                                        <div key={idx} className="border-l-2 border-primary/40 pl-3 text-[11px] italic text-slate-300">
                                                            {f.message}
                                                        </div>
                                                    ))}
                                                </div>
                                    ...
                                    {currentAssessments.length === 0 && (
                                        <p className="text-xs text-slate-500">No assessments recorded for this student yet.</p>
                                    )}
                                </div>
