...
// /**
//  * REGISTRY DISPLAY ENGINE (Tier 2/3)
//  * Rule 13: Handles both Study Mode (Student) and Selection Mode (Teacher).
//  * Rule 17: Pulls branding from Zustand.
//  */
// export function ScannedQuestionRegistry({ 
//   questions, 
//   userRole, 
...
//       {/* ── TOOLBAR ── */}
//       <div className="flex flex-col md:flex-row justify-between items-center bg-slate-900 p-8 rounded-[2.5rem] border border-white/5 gap-6 shadow-2xl">
//         <div className="flex items-center gap-4">
//           <div 
...
//           </div>
//           <div>
//             <h3 className="text-white font-black uppercase italic tracking-tight text-lg">Registry Nodes</h3>
//             <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest">{questions.length} Items Discovered</p>
//           </div>
//         </div>

//         {/* Teacher Selection UI */}
//         {isTeacher && (
//           <div className="flex items-center gap-8 bg-slate-950 p-5 rounded-2xl border border-white/10 w-full md:w-auto shadow-inner">
//             <div className="flex flex-col gap-3 min-w-[220px]">
//               <div className="flex justify-between text-[10px] font-black uppercase italic">
//                 <span className="text-slate-500">Selection Density</span>
//                 <span style={{ color: primaryColor }}>{selectionPercentage}%</span>
//               </div>
//               <Slider 
//                 value={[selectionPercentage]} 
//                 onValueChange={(val) => setSelectionPercentage(val[0])} 
...
//                 onClick={handleConfirmSelection}
//                 disabled={selectionPercentage === 0}
//                 className="text-slate-950 font-black rounded-xl px-8 h-12 uppercase text-[10px] tracking-widest shadow-xl transition-all hover:scale-105"
...
//           return (
//             <Card key={q.id} className={cn(
//               "bg-slate-900 border-white/5 rounded-[2.5rem] overflow-hidden transition-all duration-500 shadow-xl",
//               isSelected && "ring-2 border-transparent"
//             )}
//             style={isSelected ? { ringColor: primaryColor, borderColor: primaryColor } : {}}
//             >
//               <CardHeader className="bg-slate-950/50 border-b border-white/5 flex flex-row items-center justify-between p-8">
//                 <div className="flex items-center gap-4">
//                   <span className="h-8 w-8 bg-slate-900 border border-white/5 rounded-xl flex items-center justify-center text-[10px] font-black text-slate-500">
//                     #{idx + 1}
//                   </span>
//                   <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">
//                     {q.examBody} • {q.year} Registry
//                   </span>
//                 </div>
//                 {isSelected && <CheckCircle2 className="h-5 w-5 animate-pulse" style={{ color: primaryColor }} />}
//               </CardHeader>
...
//                   >
//                     {isRevealed ? <><EyeOff className="h-4 w-4" /> Conceal Logic</> : <><Eye className="h-4 w-4" /> Reveal Solution</>}
//                   </button>

//                   {isRevealed && (
//                     <div className="bg-slate-950 rounded-3xl p-8 border border-white/5 animate-in slide-in-from-top-4 duration-500 shadow-inner">
//                         <div className="flex items-center gap-2 mb-4">
//                             <div className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: primaryColor }} />
//                             <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Pedagogical Explanation</h4>
//                         </div>
//                         <p className="text-base text-white font-bold italic mb-6">"{q.correctAnswer}"</p>
...
/**
 * REGISTRY DISPLAY ENGINE (Tier 2/3)
 * Rule 13: Contextual View - Study mode for Students, Selection mode for Teachers.
 * Rule 17: Leverages Zustand for primary branding.
 * Rule 11: System Truth - Reflects the database state of scanned nodes.
 */
export function ScannedQuestionRegistry({ 
  questions, 
  userRole, 
  ...
  /**
   * Rule 9: Random Sampling Selection Logic
   * Derives a random subset of the current filter results based on percentage.
   */
  const selectedQuestions = useMemo(() => {
    if (!isTeacher || selectionPercentage === 0) return [];
    const count = Math.ceil((selectionPercentage / 100) * questions.length);
    // Sort randomly and take 'count' to ensure exam variety
    return [...questions].sort(() => 0.5 - Math.random()).slice(0, count);
  }, [selectionPercentage, questions, isTeacher]);
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest">
                {questions.length} Question Items Discovered
            </p>
          </div>
        </div>

        {/* Teacher Selection UI (Institutional Only) */}
        {isTeacher && (
          <div className="flex items-center gap-8 bg-slate-950 p-5 rounded-2xl border border-white/10 w-full md:w-auto shadow-inner">
            <div className="flex flex-col gap-3 min-w-[220px]">
              <div className="flex justify-between text-[10px] font-black uppercase italic">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {questions.map((q, idx) => {
          ...
          // ✅ FIX: Use valid CSSProperties for dynamic highlighting
          const cardStyle: CSSProperties = isSelected ? {
              borderColor: primaryColor,
              boxShadow: `0 0 20px ${primaryColor}20` // Glow in the school's primary color
          } : {};

          return (
            <Card key={q.id} className={cn(
              "bg-slate-900 border-white/5 rounded-[2.5rem] overflow-hidden transition-all duration-500 shadow-xl",
              isSelected ? "border-2" : "hover:border-white/10"
            )}
            style={cardStyle}
            >
              <CardHeader className="bg-slate-950/50 border-b border-white/5 flex flex-row items-center justify-between p-8">
                <div className="flex items-center gap-4">
                  ...
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">
                    {q.examBody || "External"} • {q.year || "Archive"} Registry
                  </span>
                </div>
                {isSelected && <CheckCircle2 className="h-5 w-5 animate-pulse" style={{ color: primaryColor }} />}
              </CardHeader>
                ...
                <div className="pt-8 border-t border-white/5 flex flex-col gap-6">
                  ...
                  {isRevealed && (
                    <div className="bg-slate-950 rounded-3xl p-8 border border-white/5 animate-in slide-in-from-top-4 duration-500 shadow-inner">
                        ...
                        <p className="text-base text-white font-bold italic mb-6">"{q.correctAnswer}"</p>
                        <p className="text-sm text-slate-400 leading-relaxed font-medium">
                            {q.explanation || "No rationale provided in registry."}
                        </p>
                    </div>
                  )}
                </div>
