...
// Define a custom interface to stop the build from failing
// "use client"
...
// // Helper function to extract config
// function getPayloadConfigFromPayload(
//   config: ChartConfig,
//   payload: unknown,
//   key: string
  ...
  (
    ...
    return (
      ...
      >
        ...
        <div className="grid gap-1.5">
          {payload
            .filter((item: RechartsItem) => item.type !== "none")
            .map((item: RechartsItem, index) => {
              const key = `${nameKey || item.name || item.dataKey || "value"}`
              const itemConfig = getPayloadConfigFromPayload(config, item, key)
